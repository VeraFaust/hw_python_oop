...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        return (
            (self.CALORIES_RUN_1 * self.get_mean_speed()
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        return (
            (self.CALORIES_WALKING_1 * self.weight
...
class Swimming(Training):
    ...
    def get_spent_calories(self) -> float:
        return (
            (self.get_mean_speed() + self.CALORIES_SWIM_1)
...
def read_package(workout_type: str, data: List) -> Training:
    ...
    trainings_types: Dict[str, Type[Training]] = {
        'SWM': Swimming,
        'RUN': Running,
        'WLK': SportsWalking
    }
    if workout_type not in trainings_types:
        message = (
            f'Неизвестный тип тренировки: {workout_type}'
            f'Существующие типы тренировок: {trainings_types}'
        )
        raise ValueError(message)
    return trainings_types[workout_type](*data)
